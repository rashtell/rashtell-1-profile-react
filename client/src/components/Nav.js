...
class NavMenuItem extends Component {
    ...
    handleClick = (id, e) => {
        ...
        navMenuItemProps.find(match => match.id === id).aClassName = "w3-bar-item w3-button w3-padding-large w3-hover-black w3-black";

        navMenuItemProps.filter(match => id !== match.id).map(single => single.aClassName = "w3-bar-item w3-button w3-padding-large w3-hover-black");
    ...
    render = () => {
        return (
            this.state.navMenuItemPropsList.map((navMenuItemProp, i) => {
                return (
                    <a
                        key={i}
                        ...
                        <i
                            className={navMenuItemProp.iClassName}>
                        </i>
                        <p>
                            {navMenuItemProp.menu}
                        </p>
                    </a>
                )
            })
...
class NavMenuItemMini extends Component {
    render() {
        ...
        return (
            navMenuItemMiniProps.map((navMenuItemMiniProp, i) => {
                return (
                    <a
                        key={i}
